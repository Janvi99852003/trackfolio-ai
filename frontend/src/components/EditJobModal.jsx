...
function EditJobModal({ job, onClose, onJobUpdated, api }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      const res = await api.put(`/jobs/${job._id}`, { company, role, status, link });
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center px-4 z-50">
      <div className="bg-slate-800 rounded-2xl shadow-xl p-8 w-full max-w-md">
        <h2 className="text-2xl font-bold text-white mb-6">Edit Job Application</h2>
